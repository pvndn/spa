...
use App\Models\Page;
use App\Models\SettingsInterface;
...
class PageController extends BaseFrontController
{
    ...
    {
        $interface = SettingsInterface::where('slug', 'dich-vu')->with('interfaceContent.interfaceItem')->first();
        $items = [];
        foreach ($interface->interfaceContent as $value) {
            if( $value->language_id == $this->currentLanguageId ) {
                $items = $value->interfaceItem;
            }
        }
        $temp = [];
        ...
        $html_src = view('front._partials._content_post_item_homepage', [
            'items' => $temp,
        ])->render();

        $this->dis['contentItem'] = $html_src;
