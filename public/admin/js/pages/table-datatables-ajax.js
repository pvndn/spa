var TableDatatablesAjax = function () {
    ...
    var handleRecords = function (options) {
        ...
        function saveRow(oTable, nRow) {
            ...
            oTable.fnUpdate('<a class="edit">Fast edit</a>', nRow, options.actionPosition, false);
            ...
            $.ajax({
                ...
                complete: function(data) {
                    ...
                    if(typeof data.responseJSON != 'undefined')
                    {
                        ...
                        {
                            Utility.showNotification(data.responseJSON.message, 'danger');
                        }
                        else
                        {
                            Utility.showNotification(data.responseJSON.message, 'success');
                        }

                    }
                    else
                    {
                        Utility.showNotification('Some error occurred!', 'danger');
                    }
                    oTable.api().ajax.reload();
                }
            });
        ...
        grid.getTableWrapper().on('click', '.fast-edit', function(e){
            e.preventDefault();
